...
- Added `read_rdata()`: reads `.RData`/`.rda` workspaces (R's `save()`
  ...
  dicts, atomic vectors with the usual NA/factor/date-time handling).
  `select=` materializes only the named objects while structurally
  skipping the rest -- the skip registers the same reference-table entries
  a full read would, so later objects sharing symbols/environments with
  skipped ones still resolve; unknown names raise `ValueError` listing
  what the workspace contains. XDR and native-binary containers,
  serialization versions 2 and 3, all compression formats; ASCII
  workspaces fail explicitly. `rdsframe dump` now detects the container
  and works on both RDS and RData; `inspect_r_file()` reports binary
  RData as `fast_supported`; `validate_rds()` points RData files at
  `read_rdata()`. Golden-file tests are generated by R 4.5.0 itself
  (`tests/data/r450/workspace_*.RData`), including a reference-alignment
  case where a selected data.frame's attribute symbols are back-references
  into entries registered while skipping an earlier one.
